...
class GameCharacter {
    constructor() {
        GameCharacterController.instance.generateModel(Configuration.instance.characterStartingPoint, Configuration.instance.characterPathLevel, Sprite.StandingFrontFacing);
    }
    static get instance() {
        if (!this._instance)
            this._instance = new GameCharacter();
        return this._instance;
    }
    static drawMoscularFootStandingFront() {
        push();
        fill(GameCharacterModel.colors.moscularFootColor);
        rect(-12, 0, 24, 17);
        pop();
    }
    static drawSoftBodyFront(translate_y = 0) {
        push();
    ...
    static mouthLayerStandingFront(layerScale = 1) {
        push();
        scale(layerScale);
        beginShape();
        vertex(-7, 10);
    ...
    static teethStandingFront(layerScale) {
        push();
        strokeWeight(0.5);
        scale(layerScale);
        line(-4, 10, -10, 4);
    ...
    static moscularFootJumpingFront() {
        push();
        fill(GameCharacterModel.colors.moscularFootColor);
        beginShape();
        vertex(0, 0);
    ...
    static drawTentacleFront(translate_y = 0, organPlace) {
        push();
        translate(0, translate_y);
    ...
    static irisPlacement(scieraPos_x, scieraPos_y, organPlace) {
        // defaults to center
    ...
    static getTentacleFrontHorizontalCoord(organPlace) {
    ...
    static drawShellDecorationFront(translate_y = 0) {
        push();
        translate(0, translate_y);
    ...
    static drawShellStructure(sprite, translate_y = 0) {
        push();
        translate(0, translate_y);
    ...
    static eye(scieraPos_x, scieraPos_y, irisPosition) {
        // eye sciera
    ...
    static shellDecorationDotProfile(x, y, dotScale, rotationDegree) {
        push();
        fill(GameCharacterModel.colors.shellDotsColor);
        translate(x, y);
        angleMode(DEGREES);
    ...
    static jumpingToTheLeft(center_x, bottom_y, rotation) {
        push();
        noStroke();
    ...
    static jumpingToTheRight(center_x, bottom_y, rotation) {
        push();
        noStroke();
        GameCharacter.changeAnchorPoint(center_x, bottom_y);
        rotate(rotation);
    ...
    static walking(center_x, bottom_y, sprite, changeAnchor = true) {
        push();
        noStroke();
        if (changeAnchor) {
            GameCharacter.changeAnchorPoint(center_x, bottom_y);
    ...
    static changeAnchorPoint(center_x, bottom_y, reverseHorizontalAxis = true) {
    ...
    static standingFrontFacing(center_x, bottom_y) {
        push();
        noStroke();
        GameCharacter.changeAnchorPoint(center_x, bottom_y);
    ...
    static jumpingFacingForward(center_x, bottom_y) {
        noStroke();
        push();
        GameCharacter.changeAnchorPoint(center_x, bottom_y);
