class Ground {
    private readonly sandDrawingArea = <DrawingArea>{
        start_x: 0,
    ...
    private readonly roadDrawingArea = <DrawingArea>{
        start_x: 0,
        end_x: Configuration.instance.visibleSceneWidth,
    ...
    private readonly footprintsDrawingArea = <DrawingArea>{
    ...
    public static get instance(): Ground {
        if (!this._instance) this._instance = new Ground();
        return this._instance;
    }

    private static drawFootPrint(center_x: number, bottom_y: number, isLeftFoot: boolean) {
        push();
        translate(center_x, bottom_y);
        if (isLeftFoot) scale(1, -1);
    ...
    public draw(): void {
        ...
        this.drawRoadFill();
        this.drawRoadLines();
        const visibleFootprints = BufferedRenderer.instance.getVisibleMovableModels<FootPrintModel>(FootPrintsController.instance.models);
        for (const model of visibleFootprints) {
            Ground.drawFootPrint(model.center_x, model.bottom_y, model.isLeftFoot);
        }
    }

    private drawRoadFill() {
        push();
