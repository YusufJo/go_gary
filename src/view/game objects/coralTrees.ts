class CoralTrees {
    private readonly totalNumber = Configuration.instance.totalSceneWidth * 0.8 / 100;
    ...
    private static _instance: CoralTrees;

    public static get instance(): CoralTrees {
        if (!this._instance) this._instance = new CoralTrees();
        return this._instance;
    }

    private static get flowersCoordinates(): CoralTreeFlower[] {
        return [
            <CoralTreeFlower>{x: -32, y: 12},
            <CoralTreeFlower>{x: -40, y: 20},
            <CoralTreeFlower>{x: -33, y: 40},
            <CoralTreeFlower>{x: -22, y: 28},
            <CoralTreeFlower>{x: -17, y: 42},
            <CoralTreeFlower>{x: -10, y: 24},
            <CoralTreeFlower>{x: -1, y: 45},
            <CoralTreeFlower>{x: 5, y: 69},
            <CoralTreeFlower>{x: 18, y: 49},
            <CoralTreeFlower>{x: 16, y: 88},
            <CoralTreeFlower>{x: 32, y: 100},
            <CoralTreeFlower>{x: 29, y: 80},
            <CoralTreeFlower>{x: 37, y: 47},
            <CoralTreeFlower>{x: 12, y: 22}
        ];
    }

    private static drawTreeBody(colors: CoralTreeColors): void {
        push();
    ...
    private static drawShinyReflections(colors: CoralTreeColors): void {
        push();
    ...
    private static drawTreeFlowers(colors: CoralTreeColors): void {
    ...
    public draw() {
        const visibleModels =
            BufferedRenderer.instance.getVisibleMovableModels<CoralTreeModel>(CoralTreesController.instance.models);
