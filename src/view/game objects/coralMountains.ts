class CoralMountains {
    private readonly landCoverage = 0.5;
    private readonly totalNumber = (Configuration.instance.totalSceneWidth - Configuration.instance.visibleDrawingArea.start_x) * this.landCoverage / CoralMountainModel.defaultWidth;
    ...
    private static _instance: CoralMountains;

    public static get instance(): CoralMountains {
        if (!this._instance) this._instance = new CoralMountains();
        return this._instance;
    }

    private static drawVariant1Layer(color: string, translate_x: number = 0) {
        push();
    ...
    private static drawVariant2Layer(color: string, translate_x: number = 0) {
        push();
        translate(translate_x, 0);
        fill(color);
        beginShape();
    ...
    public draw(): void {
        const visibleModels = BufferedRenderer.instance.getVisibleMovableModels<CoralMountainModel>(CoralMountainsController.instance.models);
